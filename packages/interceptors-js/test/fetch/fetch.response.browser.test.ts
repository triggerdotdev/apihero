...
const httpServer = new HttpServer((app) => {
  ...
  app.get("/get", (req, res) => {
    res.status(201).json({ route: "/get" }).end();
  });
});

async function prepareRuntime() {
  const context = await pageWith({
    example: path.resolve(__dirname, "fetch.response.browser.runtime.js"),
  });
...
test("requests with an HTTP request handled in the resolver", async () => {
...
test("requests with an HTTPS request handled in the resolver", async () => {
  const context = await prepareRuntime();
  const response: SerializedResponse = await context.page.evaluate((url) => {
    /**
